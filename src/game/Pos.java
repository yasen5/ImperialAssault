...
public class Pos {
        ...
        for (WallLine wallLine : Constants.wallLines) {
            if (wallLine.intersects(new FullPos((int) (getFullX() + Constants.tileSize * 0.5),
                    ...
                    new FullPos(
                            ...
                            (newY + 0.5) * Constants.tileSize), true)) {
                return false;
            }
        }
        return (Constants.tileMatrix[newY][newX] == 1 && Game.isSpaceAvailable(new Pos(newX, newY)));
    }
    ...
    public Pos getNextPosition(Directions dir) {
        if (!canMove(dir)) {
            return null;
        }
